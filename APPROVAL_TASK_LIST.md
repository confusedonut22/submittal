...
## 1. Product / rules definition — 8/11 completed
...
9. [ ] Double-after-split (DAS) needs final explicit decision in code + docs.
10. [ ] Same-value split policy needs final explicit decision in code + docs.
11. [ ] Final approved ruleset needs one authoritative, implementation-matching source of truth.

---

## 2. Frontend gameplay behavior — 6/11 completed
...
7. [ ] Button legality should be verified as fully derived from engine/store truth in all play states.
...
## 3. Math / RTP readiness — 5/11 completed
...
6. [ ] Base-game RTP must be brought to or confirmed below 98.0%.
7. [ ] Current rules should be re-evaluated to determine why recent RTP appears above target.
8. [ ] DAS / split-rule choices should be assessed for RTP impact.
9. [ ] Final publishable base RTP value for submission should be produced.
10. [ ] Side-bet RTP values should be revalidated and surfaced in final docs.
11. [ ] Player-facing RTP/disclosure text should be finalized.

---

## 4. Export / submission math artifacts — 3/7 completed
...
4. [ ] Final export path should reflect real split-capable blackjack flow.
...
## 5. RGS / backend integration boundary — 4/10 completed
...
5. [ ] Confirm whether current RGS integration is submission-grade or still primarily a mock/scaffold.
...
10. [ ] Ensure docs describing RGS flow match actual implementation and intended submission architecture.

---

## 6. Test coverage / regression confidence — 4/8 completed
...
5. [ ] Wager accounting on split/double should be fully regression-tested.
6. [ ] Multi-hand progression/order should be tested beyond the helper-level case.
7. [ ] Frontend interaction paths around split/multi-hand flow should be reviewed for broader coverage.
8. [ ] Math-side regression tests should be expanded where needed.

---

## 7. Docs / approval package consistency — 4/9 completed
...
6. [ ] Game spec should be synchronized with real implementation.
